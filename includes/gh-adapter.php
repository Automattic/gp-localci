...
class GP_LocalCI_Github_Adapter {
	 ...
	 *
	 * @param object $gh_data   Github data generated by ci adapter::get_gh_data().
	...
	public function post_suggestions_comments( $gh_data, $coverage ) {
		...
		foreach ( $coverage['new_strings'] as $string ) {
			if ( $string['suggestions'] ) {
				...
				$comment = $this->pr_suggestion_comment( $matches[1][0], $string, $string['suggestions'], $file, $sha, $status_page_url );
				if ( ! $comment ) {
					continue;
				}
				...
				if ( ! isset( $previous_comments[ $file ][ $comment['position'] ] ) ) {
					...
					if ( $this->api_patch( "/repos/$owner/$repo/pulls/comments/{$previous_comment->id}", $comment ) ) {
						$edited_comments++;
					}
				}
			}
	 ...
	 * @param string $sha              Latest commit on the PR.
	 ...
	 * @param string $pr_number  Github PR number.
	 *
	 ...
	 * @param string $owner            Github user.
	 * @param string $repo             Github repo.
	 * @param string $sha              Github commit to post about.
